...
Plan: 2 of 7 in current phase complete (04-01, 04-02 — both SUMMARY-complete); Phase 3 (03-01 through 03-06) is fully complete per ROADMAP.md.
Status: 04-01 CODE-COMPLETE and STATIC-VERIFIED. One new migration (`supabase/migrations/20260715230011_csv_import.sql`) adds `import_batches` + `symbol_mappings` (account-ownership RLS matching the `transactions` EXISTS-subquery shape), `transactions.import_batch_id`/`import_row_hash` provenance columns, a partial UNIQUE index `(account_id, import_row_hash) WHERE import_row_hash IS NOT NULL` as the DB-level idempotent-reimport backstop, and a `find_or_create_instrument` SECURITY DEFINER RPC — the single validated, authenticated-only controlled write path into the closed `instruments` table (no admin client, no permissive INSERT policy opened). `scripts/rls-isolation-test.ts` extended with two-user isolation checks for both new tables. `npx tsc --noEmit` clean; `git diff --stat` confirms exactly one new migration file and zero existing migrations touched. Live migration apply and live `npm run test:rls` run remain explicitly DEFERRED to plan 04-07 (no Docker; hosted DB push needs explicit user consent) — `npm run test:rls` was run and failed honestly with "table not found in schema cache" (the migration is not yet pushed), confirming the deferred state is real, not fabricated. 04-02 CODE-COMPLETE and STATIC-VERIFIED (executed concurrently with 04-01 on disjoint files). `src/lib/import/{types,normalize,row-hash,detect-broker}.ts` are the pure import-parsing primitives: money/quantity normalization that returns `null` (never a fabricated `0`) on unparseable input, both brokers' date formats normalizing to ISO, a stable occurrence-indexed sha256 row hash (idempotency linchpin for IMPT-05), and byte-evidence broker detection (zip magic vs CSV header sniff). `xlsx` pinned to SheetJS's CDN tarball 0.20.3 (never the CVE-carrying npm registry 0.18.5) and `papaparse` installed; both `test:import-primitives` and `test:import-parse` npm scripts registered so 04-03 needs no `package.json` edit. `npm run test:import-primitives` passes all 6 case groups and produces identical output across two runs (purity proven); `npx tsc --noEmit` clean.
Last activity: 2026-07-15 — 04-02 (import parsing primitives: types.ts, normalize.ts, row-hash.ts, detect-broker.ts, scripts/import-primitives-test.ts, xlsx+papaparse install) completed, committed as four atomic TDD commits (setup/RED/GREEN/REFACTOR). Also: 04-01 (import_batches/symbol_mappings schema, transactions provenance columns + idempotency index, find_or_create_instrument RPC, extended RLS isolation test) completed, committed as three atomic commits.

Progress: [██....] ~29% (2/7 plans in Phase 4 code-complete/static-verified: 04-01 storage + controlled-write foundation, 04-02 import parsing primitives. See each plan's own SUMMARY for detail.)
